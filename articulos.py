...
def dbconsult(var):
    ...
    cursor = db.cursor()
    f = cursor.execute("SELECT * FROM contactos WHERE nombre = '%s'"%(var))
    fila = f.fetchone()
    ...
    if (fila != None):
        ...
        contenedor1 = Label(ventana_home, text="1. Nombre: %s"%fila[0])
        contenedor2 = Label(ventana_home, text="2. Apellido: %s"%fila[1])
        contenedor3 = Label(ventana_home, text="3. Correo: %s"%fila[2])
        contenedor4 = Label(ventana_home, text="4. Pais: %s"%fila[3])
